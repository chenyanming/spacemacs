...
plugin which supercharges the keys ~f~, ~F~, ~t~ and ~T~ in the following
manners:
- these keys search for the previous or next char in the whole buffer instead
  of searching on the current line only,
- repeating the key just after a search behaves like ~,~ and ~;~. For instance
  ~f n f f~ will go to the third occurrence of the =n= character.
...
whole buffer. Immediatly after a search the keys ~f~, ~F~, ~t~ and ~T~ can be
repeated to continue searching for the next or previous characters.
...
and =evil-snipe-repeat-scope= to one of these, the default value is =whole-buffer=):
